...
public class EducationDAO extends DBContext implements Serializable {
    ...
    public List<Education> getAllEducation() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String uniName = rs.getString(3);
                String rank = rs.getString(4);
                Account a = accD.getAccountById(rs.getInt(5));
                Education edu = new Education(id, major, uniName, rank, a);
    ...
    public List<Education> getEducationbyId() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                edu.setMajor(rs.getString(2));
                edu.setUniName(rs.getString(3));
                edu.setRank(rs.getString(4));
    ...
    public boolean insert(Education edu) {
        String sql = "INSERT INTO education (major, uni_name, rank,account_id)\n"
                + "VALUES (?, ?, ? ,?)";
        try {
            PreparedStatement st = getConnection().prepareStatement(sql);
            st.setString(1, edu.getMajor());
