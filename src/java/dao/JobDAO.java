...
public class JobDAO extends DBContext implements Serializable {
    ...
    public List<Job> getAll() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Job job = new Job(id, jobType, com, des, location, yearExp, salary, acc, applyId);
                jobList.add(job);
            }
        } catch (Exception ex) {
    ...
    public boolean insert(Job newJob) {
        String sql = "insert into jobs (job_type, company_id, des, location, year_exp, salary, poster_id, apply_id) values (?,?,?,?,?,?,?,?)";
    ...
    public boolean update(Job updateJob) {
        String sql = "UPDATE jobs SET job_type = ?, company_id = ?, des = ?, location = ?, year_exp = ?,"
                + "salary = ?, poster_id = ?, apply_id = ? WHERE id = ?";
    ...
    public List<Job> search(String name, String companyId, String salaryF, String salaryT, String[] locations, String[] yearsOfExperience, int index) {
        List<Job> jobList = new ArrayList<>();
        boolean flag = false;
    ...
    public boolean apply(int applierId, int jobId, String cv_link) throws Exception {
    ...
    public List<Job> getJobByApplierId() {
        ...
        try {
            ...
            while (rs.next()) {
                Job j = new Job(rs.getInt(01), rs.getString(9), cdao.getCompanybyId(rs.getInt(2)), rs.getString(3), rs.getString(4), rs.getInt(5), rs.getDouble(6), adao.getAccountById(rs.getInt(7)), rs.getInt(8));
                list.add(j);
            }
        } catch (Exception e) {
        }
    ...
    public List<Job> pagingJobs(int index) throws SQLException {
        List<Job> list = new ArrayList<>();
        String sql = "Select * from jobs order by id offset ? rows fetch next 4 rows only";
        try {
            PreparedStatement st = getConnection().prepareStatement(sql);
...
//        System.out.println(new JobDAO().getJobbyId(2));
        ...
        String[] yearsExp = {"-1"};
        List<Job> data = dao.search("tech", "", "", "", null, yearsExp, 1);
        for (Job job : data) {
            System.out.println(job);
        }
//        System.out.println(dao.getCV());
//        System.out.println(dao.pagingJobs(1));
    }
