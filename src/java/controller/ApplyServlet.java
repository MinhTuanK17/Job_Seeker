...
public class ApplyServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int applyId = ((Account) session.getAttribute("account")).getAccountId();
        String jobId_raw = request.getParameter("jobId");
        try {
            int jobId = Integer.parseInt(jobId_raw);
            JobDAO dao = new JobDAO();
            if (dao.apply(applyId, jobId, cv_link)) {
                request.setAttribute("error7", "Successful apply this job");
                request.getRequestDispatcher("apply.jsp").forward(request, response);

            }
            request.setAttribute("error7", "Cannot apply this job");
